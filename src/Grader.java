...
public class Grader {
    ...
    private static final Map<String, Class<?>> TASKS = new HashMap<String, Class<?>>() {{
        //put("Aufgabe1", VerzahnungenGradingTest.class);
        //put("Aufgabe2", PrimfaktorenGradingTest.class);
        put("Aufgabe3", BienenGradingTest.class);
    ...
    public Grader(Path root) {
        this.root = root;
        results = TASKS.keySet().stream().collect(toMap(t -> t, t -> new Results()));
    }
    ...
    private void gradeTask(Path solution, String task) throws IOException {
        Path projectPath = solution.resolve("results-clean/" + task);
        ...
        if (compiled) {
            ...
            JavaProcessBuilder jUnitBuilder = new JavaProcessBuilder(TestRunner.class, TASKS.get(task).getName());
